...
export class Bundle extends AggregateRoot<BundleId> implements PricableAndWeightable {
  ...
  constructor(
    ...
    const bundleCreated = BundleCreatedEvent.create(id, name, description, currency, price, stock, weight, imageUrl, caducityDate, products);
    super(id, bundleCreated);
  }

  protected checkValidState(): void {
    if (!this.name || !this.description || !this.currency || !this.price || !this.stock || !this.weight || !this.imageUrl || !this.caducityDate) {
