...
export class StripeService implements IStripeService {
  ...
  constructor() {
    this.stripe = new Stripe(process.env.STRIPE_TEST_KEY, { apiVersion: '2025-02-24.acacia' });
  }
  ...
  async saveUser(userId: string, email: string): Promise<string> {
    try {
      const customer = await this.stripe.customers.create({ email: email, metadata: { userId: userId } });
      return customer.id;
    } catch (err) {}
  }
  ...
  async saveCard(userId: string, cardId: string): Promise<boolean> {
    try {
      await this.stripe.paymentMethods.attach(cardId, { customer: userId });
      return true;
    } catch (err) {}
  }
  ...
  async getCards(customerId: string): Promise<StripeResponseCard[]> {
    try {
      ...
      return paymentMethods.data.map(method => ({ id: method.id, brand: method.card.brand, last4: method.card.last4, exp_month: method.card.exp_month, exp_year: method.card.exp_year }));
    } catch (err) {}
  }

  async refundPayment(idOrder: string, costumerId: string): Promise<boolean> {
    try {
      const paymentIntent = await this.stripe.paymentIntents.list({ customer: costumerId, limit: 100 });
      const payment = paymentIntent.data.find(p => p.metadata.idOrder === idOrder);
      if (payment) {
        await this.stripe.refunds.create({ payment_intent: payment.id });
        return true;
      }
      return false;
    } catch (e) {}
  ...
  async deleteCard(userId: string, cardId: string): Promise<boolean> {
    try {
      const paymentMethods = await this.stripe.paymentMethods.list({ customer: userId, type: 'card' });
  ...
  async getOrdersNotRefundWithStripe(userId: string): Promise<string[]> {
    try {
      const paymentIntents = await this.stripe.paymentIntents.list({ customer: userId, limit: 100 });
  ...
  async getOrdersRefundWithStripe(idStripe: string): Promise<string[]> {
    try {
      const paymentIntents = await this.stripe.paymentIntents.list({ customer: idStripe, limit: 100 });
